...
4. **Push (Optional)**: Ask user for confirmation before pushing; only push if explicitly requested
...
There are few skills that are used in the project to help the user with the tasks. You can find them in the `<root>/.skills` directory.
````
